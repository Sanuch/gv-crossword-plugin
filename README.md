...
Рекомендуемый путь для обычного пользователя: установка из собранных файлов, которые публикует GitHub workflow.

### Вариант A. Установка из GitHub Release (рекомендуется)

1. Откройте вкладку Releases в репозитории.
2. Скачайте архив:
  - `crossword-chrome.zip` для Chrome/Chromium
  - `crossword-firefox.zip` для Firefox
3. Распакуйте архив в отдельную папку.

Для загрузки используйте инструкции ниже:

- Chrome: загрузите распакованную папку через `chrome://extensions`.
- Firefox: загрузите `manifest.json` из распакованной папки через `about:debugging#/runtime/this-firefox`.

### Вариант B. Установка из GitHub Actions Artifacts

Если релиз еще не создан, собранные архивы доступны в последнем успешном запуске workflow `Build Extension`.

1. Откройте вкладку Actions.
2. Выберите успешный запуск `Build Extension`.
3. Скачайте артефакт `crossword-chrome` или `crossword-firefox`.
4. Распакуйте архив и установите так же, как в варианте A.

### Вариант C. Для пользователя Chrome/Chromium (через unpacked)
...
### Вариант D. Для пользователя Firefox (временная установка)
...
### Вариант E. Локальная сборка (для разработчика)

Если нужно собрать расширение локально:
...
После этого используйте `dist/chrome` или `dist/firefox` по тем же шагам, что выше.
